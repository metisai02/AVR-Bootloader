...
void boot_main()
{
	...
	while (1)
	{
		do
		{
			UART_Receive(boot_data, 1, 1000);
			//  if ((PIND & (1 << PORTD4)) == 0)
			//    __asm__ volatile("jmp 0x0000");

		} while ((boot_data[0] != BOOT_START));
		UART_Receive(&boot_data[1], 1, 1000);
		UART_Receive(&boot_data[2], (boot_data[1] + 4), 1000);
		uint16_t converted_data = (boot_data[5] << 8) | boot_data[4];
		// check command
		switch (boot_data[2])
		{
			case BOOT_WRITE_CMD:
			d_write = (boot_write_t *)&boot_data[4];
			toggle_led();
			if (boot_write_handler(converted_data, &boot_data[6], boot_data[1]) == BOOT_OK)
			{
				boot_send_ack();
				count++;
			}
